...
function areFilteringObjEqual(firstFilteringObj, secondFilteringObj) {
    ...
    if (isEmptyConditionExpressionArrays(firstFilterConditions) &&
        ...
        for (var idx = 0; idx < firstFilterFilters.length; idx++) {
            ...
            if (!areFilteringObjEqual(filterFromFirst, filterFromSecond)) {
                return false;
            }
        }
    }
        ...
        for (var idx = 0; idx < firstFilterConditions.length; idx++) {
            ...
            if (conditionFromFirst.attributeName !== conditionFromSecond.attributeName ||
...
function willLoadExactPage(nextFirstPageNumber, nextLastPageNumber, oldPagingRefreshInput, newPagingRefreshInput, existedFirstPageNumber, existedLastPageNumber) {
    if (nextFirstPageNumber &&
        ...
        ((!!oldPagingRefreshInput && !!newPagingRefreshInput && oldPagingRefreshInput !== newPagingRefreshInput) ||
            nextFirstPageNumber > existedLastPageNumber ||
            existedFirstPageNumber > nextLastPageNumber)) {
        return true;
    }
    return false;
}
