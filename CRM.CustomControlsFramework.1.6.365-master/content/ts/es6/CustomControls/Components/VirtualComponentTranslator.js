...
var VirtualComponentTranslator = (function () {
    ...
    VirtualComponentTranslator.renderVirtualComponent = function (component, props, hostData, memHelper, purgeMemHelper) {
        ...
        var element = VirtualComponentTranslator.generateReactComponent(component, null, KEYLESS_ROOT_ID, props, hostData, memHelper, null, VirtualComponentTranslator.generateReactChildren(_getComponentKey(component, null, KEYLESS_ROOT_ID), component.getChildren(), props, hostData, memHelper, _generateFlyoutParentId(props, component)));
        if (purgeMemHelper) {
            memHelper.stopRenderFunction();
        }
        return element;
        ...
        switch (elementType.toUpperCase()) {
            ...
            case "POPUP":
                ...
                if (popupProps.type === PopupType.Root) {
                    return (React.createElement(RootPopup, { key: popupProps.key, parentCustomControlId: ownProps.controlId, id: popupProps.name, openPopup: ownProps.actions.openPopup, closePopup: ownProps.actions.closePopup }, component));
                }
                return component;
            case "FLYOUT":
        ...
        if (virtualComponents !== null) {
            if (Array.isArray(virtualComponents)) {
                ...
                for (var index = 0; index < virtualComponents.length; index++) {
                    childNodes.push(_generateReactChild(parentKey, KEYLESS_CHILD_ID + index, virtualComponents[index], props, hostData, memHelper, flyoutKey));
                }
                return childNodes;
            }
            else {
                return _generateReactChild(parentKey, KEYLESS_CHILD_ID, virtualComponents, props, hostData, memHelper, flyoutKey);
            }
        }
    };
...
function _generateReactChild(parentKey, defaultKey, virtualComponent, props, hostData, memHelper, flyoutKey) {
    if (_isVirtualComponent(virtualComponent)) {
        ...
        return VirtualComponentTranslator.generateReactComponent(component, parentKey, defaultKey, props, hostData, memHelper, newFlyoutKey, elemChildren !== null
            ? VirtualComponentTranslator.generateReactChildren(_getComponentKey(component, parentKey, defaultKey), elemChildren, props, hostData, memHelper, newFlyoutKey)
            : null);
    }
    else {
