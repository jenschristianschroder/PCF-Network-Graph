...
var InnerHorizontalScroll = (function (_super) {
    ...
    InnerHorizontalScroll.prototype._recalculateState = function () {
        ...
        if (this.props.isRTL) {
            nextArrowDisabled = scrollLeft <= SCROLL_THRESHOLD;
            prevArrowDisabled =
                this._isBrowserIEorEdge() || this._isBrowserFirefox() || this._isBrowserSafari()
                    ? Math.abs(scrollLeft) + scrollableContainer.offsetWidth + SCROLL_THRESHOLD >= scrollableContainer.scrollWidth
                    : scrollLeft + SCROLL_THRESHOLD >= scrollableDistance;
        }
        else {
            prevArrowDisabled = scrollLeft <= SCROLL_THRESHOLD;
            nextArrowDisabled =
                scrollLeft + scrollableContainer.offsetWidth + SCROLL_THRESHOLD >= scrollableContainer.scrollWidth - 1;
        }
...
var HorizontalScroll = ReactFela.connect(mapStylesToProps)(InnerHorizontalScroll);
